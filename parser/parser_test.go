...
import (
	...
	"github.com/flightman69/go-interpreter/ast"
...
func TestReturnStatements(t *testing.T) {
	...
	if len(program.Statements) != 3 {
		t.Fatalf("program.Statements does not contan 3 statements. got=%d", len(program.Statements))
	}
...
func TestIdentifierExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("program.Statemements[0] is not ast.ExpressionStatement. got=%T", program.Statements[0])
	}
...
func TestIntegerLiteralExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("exp not *ast.IntegerLiteal, got=%T", stmt.Expression)
	}

	if literal.Value != 5 {
		t.Errorf("literal.TokenLiteral not 5. got=%d", literal.Value)
	}
